# -*- coding: utf-8 -*-
"""
List/bullet parser: extracts and chunks list blocks from tender PDFs.
...
try:
    ...
    raise RuntimeError(
        "PyMuPDF is required for list extraction. Install with `pip install pymupdf`."
    ) from e
...
def extract_list_items(raw_text: str) -> List[str]:
    ...
    for line in raw_text.splitlines():
        ...
        if _BULLET_LINE_RE.match(line):
            if current:
                items.append(_normalize_text(" ".join(current)))
...
def build_list_chunks(
    ...
    *,
    ...
    for block in blocks:
        ...
        for idx, item in enumerate(extract_list_items(block.raw_text), start=1):
            l1.children.append(
                Chunk(
                    id=make_id(),
                    level="L2",
...
def parse_lists(pdf_path: str) -> List[Dict[str, Any]]:
    ...
    return [c.to_dict() for c in build_list_chunks(blocks)]
